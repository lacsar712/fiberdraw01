...
func Sample() Rec {
	return Rec{Title: "fiberdraw-sample", Body: "Draw=TG01-12 level=0.5 epoch=4", Tags: []string{"tide"}}
}

func Seed() []Rec {
	return []Rec{
		Sample(),
		{Title: "fiberdraw-seed-b", Body: "Draw=TG01-12 level=0.5 epoch=4 b", Tags: []string{"tide"}},
	}
}
...
func Enforce(title, body string, tags []string) error {
	if strings.TrimSpace(title) == "" {
		return fmt.Errorf("Draw title required")
	}
	if strings.TrimSpace(body) == "" {
		return fmt.Errorf("Draw body required")
	}
	if len(tags) == 0 {
		return fmt.Errorf("Draw tag required")
	}
	return nil
}
