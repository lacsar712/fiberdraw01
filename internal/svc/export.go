...
import (
	...
	"example.com/fiberdraw/internal/util/filedump"
	"example.com/fiberdraw/internal/util/jsonx"
)
...
func (c *Catalog) ExportJSON(ctx context.Context) ([]byte, error) {
	...
	exp := model.ExportFile{Name: "FiberDraw Tower", Age: len(recs)}
	...
	snap := Snapshot{
		...
		Records: recs,
		Export:  decoded,
		Meta:    map[string]string{"app": "fiberdraw"},
	}
